...
class TestSumOfTwoMinElementsMinFunction:
    ...
    def test_valid_int_input(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_min(arr) == -10
    ...
    def test_valid_float_input(self, array):
        arr = array
        result = msc.sum_of_two_min_elements_min(arr)
    ...
    def test_non_numeric_elements(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_min(arr) == "Массив должен содержать только числовые элементы"
    ...
    def test_insufficient_elements(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_min(arr) == "Массив должен содержать хотя бы два элемента"
...
class TestSumOfTwoMinElementsForCycle:
    ...
    def test_valid_int_input(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_for(arr) == -10

    # позитивные проверки с числами с плавающей точкой
    @pytest.mark.parametrize('array', [[4.0, 0.0, 3.0, 19.0, 492.0, -10.0, 1.0], [4, 0.0, 3, 19.0, 492, -10, 1]])
    def test_valid_float_input(self, array):
        arr = array
        result = msc.sum_of_two_min_elements_for(arr)
        assert result == -10.0 and isinstance(result, float)
    ...
    def test_non_numeric_elements(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_for(arr) == "Массив должен содержать только числовые элементы"

    # негативные проверки, что массив одержать хотя бы два элемента
    @pytest.mark.parametrize('array', [["4"], []])
    def test_insufficient_elements(self, array):
        arr = array
        assert msc.sum_of_two_min_elements_for(arr) == "Массив должен содержать хотя бы два элемента"
